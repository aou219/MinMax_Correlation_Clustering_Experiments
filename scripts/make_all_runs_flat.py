...
def make_row(path, shared, exp, p_key, p_result):
    ...
    if not complete_approx:
        complete_ilp = safe_get(complete, "sparse_ilp", "cost")
        complete_lp = safe_get(complete, "sparse_lp_relaxation", "cost")
        complete_approx = {
            "average_pivot_approximation": safe_ratio(safe_get(complete, "pivot", "average_cost"), complete_ilp),
            "lp_relaxation_ratio": safe_ratio(complete_lp, complete_ilp),
        ...
        if complete_approx.get("average_pivot_approximation") is None:
            complete_approx["average_pivot_approximation"] = safe_ratio(
                safe_get(complete, "pivot", "average_cost"),
                complete_ilp,
            )
        if complete_approx.get("lp_relaxation_ratio") is None:
            complete_approx["lp_relaxation_ratio"] = safe_ratio(
                complete_lp,
                complete_ilp,
            )

    edge_ilp_without4 = safe_get(edge, "sparse_ilp", "without_4_cycles", "cost")
    edge_ilp_with4 = safe_get(edge, "sparse_ilp", "with_4_cycles", "cost")
    edge_lp_without4 = safe_get(edge, "sparse_lp_relaxation", "without_4_cycles", "cost")
    edge_lp_with4 = safe_get(edge, "sparse_lp_relaxation", "with_4_cycles", "cost")
    edge_pivot_average = safe_get(edge, "pivot", "average_cost")
    edge_approx = approx or {}

    if not edge_approx and edge_ilp_with4 is not None:
        edge_approx = {}

    edge_approx.pop("best_pivot_approximation_with_4_cycles", None)
    edge_approx.pop("best_pivot_approximation_without_4_cycles", None)
    edge_approx["average_pivot_approximation_without_4_cycles"] = safe_ratio(
        edge_pivot_average,
        edge_ilp_without4,
    )
    edge_approx["average_pivot_approximation_with_4_cycles"] = safe_ratio(
        edge_pivot_average,
        edge_ilp_with4,
    )
    edge_approx["sparse_lp_to_ilp_ratio_without_4_cycles"] = safe_ratio(
        edge_lp_without4,
        edge_ilp_without4,
    )
    edge_approx["sparse_lp_to_ilp_ratio_with_4_cycles"] = safe_ratio(
        edge_lp_with4,
        edge_ilp_with4,
    )
    edge_approx["lp_relaxation_ratio_with_4_cycles"] = edge_approx[
        "sparse_lp_to_ilp_ratio_with_4_cycles"
    ]
    edge_approx["bad_triangle_primal_ratio"] = safe_ratio(
        safe_get(edge, "bad_triangle_lp_bounds", "primal_cost"),
        edge_ilp_with4,
    )
    edge_approx["bad_triangle_dual_ratio"] = safe_ratio(
        safe_get(edge, "bad_triangle_lp_bounds", "dual_cost"),
        edge_ilp_with4,
    )
    edge_approx["min_disjoint_bad_triangle_ratio"] = safe_ratio(
        safe_get(edge, "bad_triangles", "min_edge_disjoint_count"),
        edge_ilp_with4,
    )
    edge_approx["max_disjoint_bad_triangle_ratio"] = safe_ratio(
        safe_get(edge, "bad_triangles", "max_edge_disjoint_count"),
        edge_ilp_with4,
    )
    ...
    row = {
        ...
        "complete_ilp_cost": safe_get(complete, "sparse_ilp", "cost"),
        "complete_lp_cost": safe_get(complete, "sparse_lp_relaxation", "cost"),
        ...
        "edge_ilp_without4_cost": edge_ilp_without4,
        "edge_ilp_with4_cost": edge_ilp_with4,
        "edge_bad_4_cycles_count": safe_get(edge, "sparse_ilp", "with_4_cycles", "bad_4_cycles_count"),
        "same_clustering_4_cycle": safe_get(edge, "sparse_ilp", "same_clustering_4_cycle"),

        "edge_lp_without4_cost": edge_lp_without4,
        "edge_lp_with4_cost": edge_lp_with4,
        ...
        "edge_average_pivot_approx_without4": edge_approx.get("average_pivot_approximation_without_4_cycles"),
        "edge_average_pivot_approx_with4": edge_approx.get("average_pivot_approximation_with_4_cycles"),
        "edge_lp_ratio_without4": edge_approx.get("sparse_lp_to_ilp_ratio_without_4_cycles"),
        ...
        "edge_all_pairs_ilp_cost": safe_get(edge, "all_pairs_ilp", "cost"),
        "edge_all_pairs_lp_cost": safe_get(edge, "all_pairs_lp_relaxation", "cost"),
        "edge_all_pairs_lp_to_ilp_ratio": edge_approx.get("all_pairs_lp_to_ilp_ratio"),
        "edge_all_pairs_ilp_runtime_seconds": safe_get(edge, "all_pairs_ilp", "runtime_seconds"),
        "edge_all_pairs_lp_runtime_seconds": safe_get(edge, "all_pairs_lp_relaxation", "runtime_seconds"),
        "edge_all_pairs_ilp_optimal": safe_get(edge, "all_pairs_ilp", "optimal"),
